...
# Assuming all_movies_regional_release_dates and all_movies are already loaded

# Rename the 'language' column to 'original_language' for consistency
all_movies_regional_release_dates <- all_movies_regional_release_dates %>%
...
# Select the columns you want to keep in the final dataset
# For example, you might want to keep one of the 'original_language' columns
merged_data <- merged_data %>%
  select(movie_id, title.x, release_year, popularity, original_language = original_language.x, poster_path, country, release_date)
...
# Birden fazla posterde analiz yapalım
...
# Poster klasörünü belirleyelim
poster_folder <- "posters/"
poster_files <- list.files(poster_folder, full.names = TRUE)

# Tüm filmler için baskın rengi ve yılı bulalım
year_colors <- data.frame(year = integer(),
                          r = numeric(), g = numeric(), b = numeric(), hex = character())

for (file in poster_files) {
  dominant_colors <- extract_colors(file, n_colors = 1)  # En baskın rengi al
  movie_name <- gsub(".jpg", "", basename(file))
  
  # Film yılını datasetten alalım
  movie_year <- df %>% filter(grepl(movie_name, title, ignore.case = TRUE)) %>% pull(release_year)
  
  # Eğer yıl bilgisi varsa
  if (length(movie_year) > 0) {
    dominant_colors$hex <- rgb(dominant_colors$r / 255, 
                               dominant_colors$g / 255, 
                               dominant_colors$b / 255)
    
    dominant_colors$year <- movie_year[1]  # Yılın ilk değerini al
    year_colors <- bind_rows(year_colors, dominant_colors)
  }
}

# Yıllara göre baskın renklerin ortalamasını alalım (REPLACED summarise() -> reframe())
...
ggplot(year_colors_grouped, aes(x = year, y = r, color = hex)) +
  geom_point(size = 4, alpha = 0.8) +  # Daha büyük ve saydam noktalar
  geom_smooth(method = "loess", color = "blue", se = FALSE) +  # Trend çizgisi ekle
  scale_color_identity() +  # Doğrudan hex kodlarını renk olarak ata
  theme_minimal() +
...
extract_colors <- function(image_path, n_colors = 3) {  
  img <- load.image(image_path)  # Görseli yükle
  
  # Görseli küçültüp işlem hızını artır
  img_resized <- resize(img, size_x = 300, size_y = 300)  

  # RGB piksel verisini al
  img_data <- as.data.frame(as.array(img_resized))
  img_data <- img_data[, 1:3]  # Sadece RGB kanallarını al
  names(img_data) <- c("r", "g", "b")  

  # RGB değerlerini 0-255 aralığına getir
  img_data$r <- img_data$r * 255
  img_data$g <- img_data$g * 255
  img_data$b <- img_data$b * 255

  # Benzersiz renklerin sayısını belirle
  unique_colors <- nrow(unique(img_data))

  # Eğer benzersiz renk sayısı seçilen `n_colors` değerinden küçükse, `n_colors`'ı ayarla
  kmeans_clusters <- min(n_colors, unique_colors)

  # K-Means ile baskın renkleri belirle (minimized n_colors)
...
extract_colors <- function(image_path, n_colors = 3) {  
  ...
  # Tekrar eden renkleri kaldır ve benzersiz renkleri bul
  unique_colors <- unique(img_data)

  # Eğer benzersiz renk sayısı `n_colors`'tan azsa, onu otomatik ayarla
  kmeans_clusters <- min(n_colors, nrow(unique_colors))

  # Eğer yalnızca 1 renk varsa, doğrudan döndür
  if (kmeans_clusters == 1) {
    return(data.frame(r = unique_colors$r, g = unique_colors$g, b = unique_colors$b))
  }

  # K-Means ile baskın renkleri belirle (sadece uygun sayıda küme kullan)
