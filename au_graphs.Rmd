...
highchart() %>%
  ...
  hc_add_series(name = "Popularity", data = top_10_movies$popularity) %>%
  hc_add_series(name = "Academy Award", data = top_10_movies$academy_award, type = 'line') %>%
  hc_add_series(name = "Academy Nominated", data = top_10_movies$academy_nominated, type = 'line') %>%
  hc_add_series(name = "Palme D'Or", data = top_10_movies$palme_dor, type = 'line') %>%
  hc_add_series(name = "Golden Lion", data = top_10_movies$golden_lion, type = 'line') %>%
...
highchart() %>%
  ...
  hc_add_series(name = "Average Popularity", data = popularity_by_year$avg_popularity) %>%
...
language_distribution <- all_movies_languages %>%
  count(language)

# Create a bar chart to show the distribution of languages
highchart() %>%
  hc_chart(type = 'column') %>%
  hc_xAxis(categories = language_distribution$language) %>%
  hc_add_series(name = "Number of Movies", data = language_distribution$n) %>%
  hc_title(text = "Language Distribution of All Movies") %>%
  hc_subtitle(text = "Languages of All Movies in the Dataset")
